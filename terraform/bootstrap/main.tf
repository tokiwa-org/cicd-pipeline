terraform {
  ...
  # backend "s3" {
  #   bucket         = "cicd-pipeline-terraform-state-ACCOUNT_ID"
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        ...
        Resource = "arn:aws:s3:::${var.project_name}-*-pipeline-artifacts-${local.account_id}/*"
      },
      {
        Effect = "Allow"
        Action = [
      ...
      },
      {
        ...
        Resource = "arn:aws:iam::${local.account_id}:role/${var.project_name}-*-ecs-*"
        Condition = {
          StringEquals = {
